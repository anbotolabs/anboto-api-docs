...
```json
{
  ...
  "exchange": 5,
  "asset_class": 2,
  "open_interest": 123456.5,
  "open_interest_value": 7500000000.0,
  "timestamp": 1783328775705
}
```

Open interest stream for a derivatives symbol (`{"topic":"open_interest","exchange":"okx","symbol":"BTC/USDT:USDT","method":"subscribe"}`). `open_interest` is in contracts/base units as reported by the exchange; `open_interest_value` is the quote-currency value when the exchange provides it. Currently collected for **OKX** (native stream); other exchanges reject the subscription. Same symbol requirement and per-connection cap as the other market data topics.
